...
class Transactions extends React.Component<{}, any> {
  ...
  render() {
    ...
    return (
      ...
      <Menu.Button className=" bg-secondary dark:bg-dark-secondary p-1 rounded  text-[#ffffff] focus:outline-none  py-1 px-2 focus:ring-white flex font-bold">{this.state.transaction_type >= 0? this.state.transactions_types[this.state.transaction_type+1].label: "All transactions"}                         <MdKeyboardArrowDown className="mt-1 ml-2"/>
</Menu.Button>
      <Menu.Items className="absolute right-0  w-72 mt-2 origin-top-right bg-white  rounded drop-shadow-2xl ring-1 ring-black ring-opacity-5 focus:outline-none z-10">
        ...
        {this.state.transactions_types.map((ttype:any) => (
                    <Menu.Item onClick={() => {this.handleType(ttype.value)}}>
        ...
        <Tab.Panels className="mt-2">
          <Tab.Panel
          key="txs" className="text-black dark:text-white"><TransactionsTable type={this.state.transaction_type} /></Tab.Panel>
          <Tab.Panel
          key="blocks" className="text-black dark:text-white"><LatestBlocksTable/></Tab.Panel>
