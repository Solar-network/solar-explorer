...
import { MdArrowBackIos, MdArrowForwardIos, MdFollowTheSigns } from "react-icons/md";

const { lang } = i18n;
const { navigation } = lang;
type WalletsTableState = {wallets:Array<any>, interval: any, supply: number, meta: any,  currentPage: number, pager: Array<any>, loading:boolean}

class WalletsTable extends React.Component<{}, WalletsTableState> {
  ...
  componentDidMount() {
    ...
    explorer.on(blockchain).core.get("blockchain").then((b) => {
      this.setState({supply:b.body.data.supply});
    });
    this.getData(1, true);
  ...
  getData(page: number = this.state.currentPage, skeleton: boolean = false) {
    let blockchain = this.context;
      ...
      if (skeleton) {this.setState({wallets: []})}
      ...
      .then((d) => {
        this.setState({ wallets: d.body.data, meta: d.body.meta});
  ...
  render() {
    return (
      <div>
          ...
          <tbody>
            {!this.state.loading && this.state.wallets.map((wallet) => (
              <tr className="bg-quartish dark:bg-dark-quartish dark:hover:bg-dark-hoverish dark:even:bg-dark-evenish hover:bg-hoverish cursor-pointer even:bg-evenish w-full h-10 z-10">
                <td>
                <LongWallet id={wallet.address} className="mx-auto"/>
                </td>
                <td>-</td>
                ...
                <td className="text-center">
                {((100 * wallet.balance) / this.state.supply).toFixed(2)}%
                </td>
                <td>
        ...
        {this.state.wallets.length == 0 && this.state.loading == false &&
